...
if __name__=='__main__':
    ...
    name  = input()
    print("Hello "+name+". We are going to capture "+str(max_images)+" images of your face for training.")
    ...
    data = []
    current_frame_number = 0  # current frame number

    while True:    # retrieve the ret (boolean) and frame from camera
        ret, frame = cam.read()
        # if the camera is working fine, we proceed to extract the face
        if ret == True:
            # convert the current frame from rgb to grayscale
            ...
            for (x, y, w, h) in faces:
                ...
                if current_frame_number % 10 == 0 and len(data) < max_images:
                    data.append(fc)
                    cv2.imwrite('images/' + name + '_' + str(len(data)) + ".jpg", gray[y:y + h, x:x + w])
                    print('captured',len(data))
                # for visualization, draw a rectangle around the face
                # in the image
                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
            current_frame_number += 1  # increment the current frame number
            cv2.imshow('frame', frame)  # display the frame

            # if the user presses the escape key (ID: 27) or the number of images hits 20, we stop recording.
            if cv2.waitKey(1) == 27 or len(data) >= max_images:
                break
        else:
